...
export default function ClientsItemsEditPage() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { name: 'Müşteri Öğeleri', to: '/admin/clients/items' },
      { name: 'Müşteri Öğesi Düzenle' }
    ]);
  }, [setBreadcrumbs]);
  ...
  const fetchClient = async () => {
    try {
      setFetching(true);
      const { data, error } = await api.clientsItems.getById(id!);
      ...
      if (client.image_path) {
        setOriginalImage(client.image_path);
      }
    } catch (error) {
      console.error('Error fetching client:', error);
      Swal.fire('Hata', 'Müşteri yüklenemedi', 'error');
      navigate('/admin/clients/items');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      Swal.fire('Hata', 'Lütfen müşteri adını girin', 'error');
      return;
    }
    ...
    try {
      ...
      if (file) {
        ...
        const filename = `client-${timestamp}-${Math.random().toString(36).slice(2)}.${extension}`;
      ...
      const { error } = await api.clientsItems.update(id!, payload);
      if (error) throw error;

      Swal.fire('Başarılı', 'Müşteri öğesi başarıyla güncellendi', 'success');
      navigate('/admin/clients/items');
    } catch (error: any) {
      console.error('Error updating client item:', error);
      Swal.fire('Hata', error?.message || 'Müşteri öğesi güncellenemedi', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <FormLayout
        title="Müşteri Öğesi Düzenle"
        subtitle="Müşteri öğesi bilgilerini güncelleyin"
        backUrl="/admin/clients/items"
      >
